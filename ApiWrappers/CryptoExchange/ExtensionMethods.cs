...
namespace CryptoExchange.Net
{
    ...
    public static class ExtensionMethods
    {
        ...
        /// <summary>
        /// Validates an int is one of the allowed values or get max
        /// </summary>
        /// <param name="value">Value of the int</param>
        /// <param name="argumentName">Name of the parameter</param>
        /// <param name="allowedValues">Allowed values</param>
        public static int ValidateIntValuesOrMax(this int? value, string argumentName, params int[] allowedValues)
        {
            if (value.HasValue)
            {
            ...
            return allowedValues.OrderBy(x => x).Last();
        }
        ...
        /// <summary>
        /// Decompress using Zlib stream
        /// </summary>
        /// <param name="data"></param>
        /// <returns></returns>
        public static ReadOnlyMemory<byte> DecompressZlib(this ReadOnlyMemory<byte> compressedData)
        {
            // Convert ReadOnlyMemory to byte[]
            byte[] buffer = compressedData.ToArray();

            using (var input = new MemoryStream(buffer))
            {
                // Skip the first 2 bytes of the zlib header (RFC1950)
                // They contain compression method and flags
                if (input.ReadByte() == -1 || input.ReadByte() == -1)
