...
namespace CryptoExchange.Net.Clients
{
    ...
    public abstract class RestApiClient : BaseApiClient, IRestApiClient
    {
        ...
        /// <param name="options">The base client options</param>
        /// <param name="apiOptions">The Api client options</param>
        public RestApiClient(ILogger logger, HttpClient? httpClient, string baseAddress, RestExchangeOptions options, RestApiOptions apiOptions, RequestFactory requestFactory)
            : base(logger,
                apiOptions.OutputOriginalData ?? options.OutputOriginalData,
                apiOptions.ApiCredentials ?? options.ApiCredentials,
                baseAddress,
        ...
        protected virtual async Task<WebCallResult<T>> SendAsync<T>(
            ...
            int? weight = null) where T : class
        {
            definition.Proxy = ClientOptions.Proxy;
        ...
        {
            // DEBUG: Print request construction details
            Console.WriteLine($"[DEBUG] RestApiClient.CreateRequest - RequestId: {requestId}");
            Console.WriteLine($"[DEBUG] RestApiClient.CreateRequest - BaseAddress: {baseAddress}");
            Console.WriteLine($"[DEBUG] RestApiClient.CreateRequest - Definition Path: {definition.Path}");
            Console.WriteLine($"[DEBUG] RestApiClient.CreateRequest - Definition Method: {definition.Method}");
            ...
            Console.WriteLine($"[DEBUG] RestApiClient.CreateRequest - Initial URI: {uri}");
            ...
            Console.WriteLine($"[DEBUG] RestApiClient.CreateRequest - Final URI after auth and params: {uri}");
            ...
            IRateLimitGate? gate = null)
        {
            ...
            try
            {
                if (ClientOptions.Proxy != null)
                    paramString += $" via proxy [{ClientOptions.Proxy.Host}:{ClientOptions.Proxy.Port}]";
            }
            catch { }
        ...
        protected virtual async Task<WebCallResult<T>> GetResponseAsync<T>(
            ...
            CancellationToken cancellationToken)
        {
            // DEBUG: Print the actual request URL being sent
            Console.WriteLine($"[DEBUG] RestApiClient.GetResponseAsync - Sending request to: {request.Uri}");
            Console.WriteLine($"[DEBUG] RestApiClient.GetResponseAsync - Request method: {request.Method}");
            Console.WriteLine($"[DEBUG] RestApiClient.GetResponseAsync - Request ID: {request.RequestId}");
