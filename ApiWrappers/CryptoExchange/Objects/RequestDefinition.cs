...
namespace CryptoExchange.Net.Objects
{
    ...
    public class RequestDefinition
    {
        ...
        public RequestDefinition(string path, HttpMethod method, ApiProxy? apiProxy = null)
        {
            Path = path;
            Method = method;
            Proxy = apiProxy;
        ...
        {
            return $"[Proxy: {(Proxy?.ToString() ?? "No Proxy")} ] {Method} {Path}{(Authenticated ? " authenticated" : "")}";
        }
    }
}
