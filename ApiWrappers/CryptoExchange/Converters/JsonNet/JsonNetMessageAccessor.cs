...
namespace CryptoExchange.Net.Converters.JsonNet
{
    ...
    public class JsonNetByteMessageAccessor : JsonNetMessageAccessor, IByteMessageAccessor
    {
        ...
        public CallResult Read(ReadOnlyMemory<byte> data)
        {
            ...
            try
            {
                using var rawStream = MemoryMarshal.TryGetArray(data, out var arraySegment)
            ...
            {
                // Если не удалось декомпрессировать, пробуем как обычный JSON
                try
                {
                ...
                {
                    IsJson = false;
                    return new CallResult(new ServerError("JsonError: " + ex2.Message));
                }
            }
        }
        /// <inheritdoc />
