...
namespace CryptoExchange.Net.RateLimiting.Guards
{
    ...
    public class RateLimitGuard : IRateLimitGuard
    {
        ...
        /// <summary>
        /// Apply guard per host per proxy
        /// </summary>
        public static Func<RequestDefinition, string, SecureString?, string> PerHostPerProxy { get; } = new Func<RequestDefinition, string, SecureString?, string>((def, host, key) => host + def.Proxy?.ToString() ?? string.Empty);
        /// <summary>
        ...
        /// <summary>
        /// Apply guard per endpoint per proxy
        /// </summary>
        public static Func<RequestDefinition, string, SecureString?, string> PerEndpointPerProxy { get; } = new Func<RequestDefinition, string, SecureString?, string>((def, host, key) => def.Path + def.Method + def.Proxy?.ToString() ?? string.Empty);
        /// <summary>
        ...
        private readonly Dictionary<string, RateLimitTrackerMetadata> _trackerMetadata;
        ...
        /// <inheritdoc />
        public string Name { get; set; } = "RateLimitGuard";
        ...
        /// <param name="name">RateLimitGuard Name</param>
        public RateLimitGuard(Func<RequestDefinition, string, SecureString?, string> keySelector, IGuardFilter filter, int limit, TimeSpan timeSpan, RateLimitWindowType windowType, double? decayPerTimeSpan = null, int? connectionWeight = null, string? name = null)
            : this(keySelector, new[] { filter }, limit, timeSpan, windowType, decayPerTimeSpan, connectionWeight, name)
        {
        }
        ...
        /// <param name="connectionWeight">The weight of a new connection</param>
        /// <param name="name">RateLimitGuard Name</param>
        public RateLimitGuard(Func<RequestDefinition, string, SecureString?, string> keySelector, IEnumerable<IGuardFilter> filters, int limit, TimeSpan timeSpan, RateLimitWindowType windowType, double? decayPerTimeSpan = null, int? connectionWeight = null, string? name = null)
        {
            _filters = filters;
            _trackers = new Dictionary<string, IWindowTracker>();
            _trackerMetadata = new Dictionary<string, RateLimitTrackerMetadata>();
        ...
        public LimitCheck Check(RateLimitItemType type, RequestDefinition definition, string host, SecureString? apiKey, int requestWeight)
        {
            ...
            if (!_trackers.TryGetValue(key, out var tracker))
            {
                ...
                _trackerMetadata.Add(key, new RateLimitTrackerMetadata
                {
        ...
        public Dictionary<string, RateLimitCurrentState> GetAllCurrentStates()
        {
            ...
            {
                //Force update Current in trackers
                t.Value.UpdateTracker();
