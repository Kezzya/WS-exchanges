...
namespace CryptoExchange.Net.RateLimiting
{
    ...
    public class RateLimitGate : IRateLimitGate
    {
        ...
        /// <inheritdoc />
        public async Task<Dictionary<string, Dictionary<string, RateLimitCurrentState>>> GetCurrentRateLimitStatesAsync()
        {
            await _semaphore.WaitAsync().ConfigureAwait(false);
            try
            {
                //Gate: {[guard: [states]]}
                var result = new Dictionary<string, Dictionary<string, RateLimitCurrentState>>();
                foreach (var guard in _guards)
                {
                    result[guard.Name] = guard.GetAllCurrentStates();
                }
